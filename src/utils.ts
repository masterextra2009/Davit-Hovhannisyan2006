...
// Список — не претендует на полноту всех русских имён, только самые
// распространённые (для персонализации экрана приветствия: мужской или
// женский персонаж). Имя — основной признак; отчество ("-ович"/"-евич",
// "-овна"/"-евна") — резервный, если по имени определить не вышло
// (иностранное имя, редкое, опечатка и т.п.).
const MALE_FIRST_NAMES = new Set([
  'александр', 'алексей', 'анатолий', 'андрей', 'антон', 'аркадий', 'артём', 'артем', 'артур',
  'богдан', 'борис', 'вадим', 'валентин', 'валерий', 'василий', 'виктор', 'виталий', 'владимир',
  'владислав', 'вячеслав', 'геннадий', 'георгий', 'герман', 'глеб', 'григорий', 'даниил', 'данила',
  'денис', 'дмитрий', 'евгений', 'егор', 'ефим', 'захар', 'иван', 'игнат', 'игорь', 'илья',
  'кирилл', 'константин', 'лев', 'леонид', 'макар', 'максим', 'марат', 'марк', 'матвей', 'михаил',
  'никита', 'николай', 'олег', 'павел', 'пётр', 'петр', 'роберт', 'родион', 'роман', 'ростислав',
  'руслан', 'савва', 'святослав', 'семён', 'семен', 'сергей', 'станислав', 'степан', 'тарас',
  'тимофей', 'тимур', 'тихон', 'фёдор', 'федор', 'феликс', 'филипп', 'эдуард', 'юрий', 'ярослав',
]);

const FEMALE_FIRST_NAMES = new Set([
  'августа', 'аглая', 'агния', 'аделина', 'александра', 'алевтина', 'алина', 'алиса', 'алла',
  'анастасия', 'ангелина', 'анжела', 'анна', 'антонина', 'валентина', 'валерия', 'вера', 'вероника',
  'виктория', 'галина', 'дарья', 'диана', 'ева', 'евгения', 'екатерина', 'елена', 'елизавета',
  'жанна', 'зинаида', 'зоя', 'инга', 'инна', 'ирина', 'карина', 'кира', 'клавдия', 'кристина',
  'ксения', 'лариса', 'лидия', 'лилия', 'любовь', 'людмила', 'майя', 'маргарита', 'марина', 'мария',
  'марта', 'милана', 'надежда', 'наталья', 'наталия', 'нина', 'оксана', 'олеся', 'ольга', 'полина',
  'раиса', 'регина', 'римма', 'роза', 'светлана', 'снежана', 'софия', 'софья', 'стелла', 'таисия',
  'тамара', 'татьяна', 'ульяна', 'юлия', 'яна', 'ярослава',
]);

export type DetectedGender = 'male' | 'female';

// Разбирает полное ФИО, ищет первое слово в списках имён; если не нашлось —
// проверяет второе слово (обычно отчество) на характерное окончание.
// Если ничего не опознано — возвращает запасной вариант по умолчанию, а не
// падает и не показывает третий "нейтральный" персонаж (клиент разрешил
// оба варианта: нейтральный или просто один из двух по умолчанию).
export function detectGenderFromFullName(fullName: string): DetectedGender {
  const words = fullName.trim().toLowerCase().split(/\s+/).filter(Boolean);
  const firstName = words[0];
  const patronymic = words[1];
