...
// Показываемые сейчас акции отдаём КОМУ УГОДНО, без входа: их лента висит на
// экране входа приложения, где человек ещё не вошёл (решение Давида
// 17.09.2026). Ничего личного в них нет — это то же, что каждый клиент и так
// видит на Главной. Скрытые акции (all=1) и любая правка по-прежнему только
// администратору.
if ($action === 'list' && ($_GET['all'] ?? '') === '') {
    require_method('GET');
    // Ограничение на случай, если по открытому адресу начнут долбить:
    // чтение дешёвое, но бесплатным его делать незачем.
    $RATE_LIMIT_MAX = 120;
